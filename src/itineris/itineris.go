...
/*
GetParamAsTypeUnsafe returns a parameter value casted to a specified type, ignoring error.
*/
func (prm *ApiParams) GetParamAsTypeUnsafe(key string, typ reflect.Type) interface{} {
	v, _ := reddo.Convert(prm.GetParam(key), typ)
	return v
}
...
var (
	ResultOk             = NewApiResult(StatusOk).SetMessage("Ok")
